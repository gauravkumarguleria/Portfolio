  ...
  >
    ...
    <div className="flex flex-wrap justify-center gap-10">
      {SkillsInfo.map((category) => (
        ...
        >
          {/* Category Title */}
          <h3 className="text-[clamp(1.5rem,2.5vw,2rem)] font-extrabold text-purple-700 mb-8 text-center drop-shadow-[1px_1px_0px_#000]">
            {category.title} ⚡
          </h3>
          ...
          >
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 w-full">
              {category.skills.map((skill) => (
                <div
                  key={skill.name}
                  className="flex flex-col items-center justify-center bg-yellow-100 border-4 border-black rounded-2xl py-4 px-3 sm:py-5 sm:px-4 hover:bg-yellow-200 hover:scale-105 transition-all shadow-[2px_2px_0px_#000]"
                >
                  <img
                    src={skill.logo}
                    alt={`${skill.name} logo`}
                    className="w-10 h-10 sm:w-12 sm:h-12 mb-2 drop-shadow-[1px_1px_0px_#000]"
                  />
